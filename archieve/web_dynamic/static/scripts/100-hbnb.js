$(document).ready(function () {
	...
	$('INPUT[type="checkbox"]').change(function() {
		if ($(this).is(':checked')) {
			...
			if ($(this).attr('id') === "Amenity") {
				amenities[$(this).attr('data-id')] = $(this).attr('data-name');
			} else {stateAndCity[$(this).attr('data-id')] = $(this).attr('data-name');}
		} else {
			if ($(this).attr('id') === "Amenity") {
				delete amenities[$(this).attr('data-id')];
			}  else {delete stateAndCity[$(this).attr('data-id')];}
		}
		
		$('.amenities H4').text(Object.values(amenities).join(', '));
		$('.locations H4').text(Object.values(stateAndCity).join(', '));
	});
